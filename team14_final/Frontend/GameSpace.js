...
const GameSpace = () => {
  ...
  const [selectedConsole, setSelectedConsole] = useState(null); // State for selected console
  ...
  const selectConsole = (consoleId) => {
    const console = consoles.find((g) => g.id === gameId);
    if(console) {
  ...
  useEffect(() => {
    const fetchConsoles = async () => {
      try {
        const response = await fetch("http://localhost:8081/listConsoles");
        const data = await response.json();
        setGames(data[0].games); // Access the nested games array
      } catch (error) {
        console.error("Error fetching games data:", error);
                ...
                <div className="col" key={index}>
                  <div className="card shadow-sm custom-card">
                    ...
                    <div className="card-body">
                      ...
                      {/* Button or link to console page */}
                    </div>
                  </div>
                </div>
              ))
        ...
        <main>
          <section className="subHeader">
            <div className="welcome-banner">
              <div className="row py-lg-5">
                <div className="col-lg-6 col-md-8 mx-auto">
                  <h1 id="gameTitle">{selectedConsole.title}</h1>
                </div>
              </div>
            </div>
          </section>
              ...
              <div id="col1">
                <div className="image">
                  <img
                    ...
                    alt={selectedConsole.title}
                  />
                </div>
                <div id="details">
                  <div id="platform1">{selectedConsole.platforms}</div>
                  <div id="releaseDate1">{selectedConsole.release_date}</div>
                  <div id="genre1">{selectedConsole.genre}</div>
                  <div id="rating1">{selectedConsole.rating}</div>
                  <div id="gameDev1">{selectedConsole.developer}</div>
                  <div id="publisher1">{selectedConsole.publisher}</div>
                </div>
              </div>
              <div id="col2">
                <div id="card2">
                  <p id="premise1" className="premise-text">
                    {selectedConsole.premise}
                  </p>
                </div>
                <div id="comments-header">
                  <h2>
